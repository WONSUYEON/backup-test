...
설명 : 숫자데이터를 처리하기 위한 숫자관련 함수를 알아보자
    테이블 생성시 number 타입으로 선언된 컬럼에 저장된
    데이터를 대상으로 한다
*************************/
...
DUAL 테이블 
    ...
    오라클에서 자동으로 생성되는 임시테이블이다.
    varchar2(1)로 정의된 dummy라는 단 하나의 칼럼으로 구성되어있다.
*/
select * from dual;
...
--첫번째 항목 : 소수이하 6자리까지 표현하므로 7을 올림처리하여 0.123457 출력
--두번째 항목 : 소수이하 4자리까지 표현하므로 1을 버림처리하여 2.3456 출력

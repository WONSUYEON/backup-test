...
/* 
concat()
문자열1과 문자열2를 서로 연결해서 출력하는 함수

    사용법 1 : select concat('문자열1', '문자열2') from dual;
    사용법 2 : select '문자열1' || '문자열2' from dual;
*/
...
select * from employees where first_name ='John';--3개인출
...
select 
    ...
    lpad('good', 9)
from dual;--d인자를 2개만 주는 경우 빈자리는 공백으로 채워진다

/*
 trim() : 공백을 제거할때 사용한다.
    형식] trim([leading | trailing | both] 제거할 문자 from 칼럼)
    -leading : 왼쪽에서 제거함
    -trailing : 오른쪽에서 제거함
    -both : 양쪽에서 제거함. 설정값이 없으면 both가 디폴트임.
    [주의1] : 양쪽끝의 문자만 제거되고, 중간에 있는 문자는 제거되지 않음.
    [주의2] : '문자'만 제거할수 있고, '문자열'은 제거할수 없다. 에러발새됨
    
 단 중간의 공백은 제거되지 않는다.
옵션에 따라 좌측, 우측, 양쪽의 문자를 제거할 수 있다. 문자열은 제거할수 없다. 
형식
...
select 
    ltrim(' 좌측공백제거 ')ltrim1,
    ltrim(' 좌측문자열제거', '좌측') ltrim2, /* 이경우 스페이스(공백)이 포함된 문자열이므로 삭제안됨*/
...
substr() : 문자열의 시작인덱스부터 길이만큼 잘라서 문자열을 출력한다.
    형식] select substr(컬럼, 시작인덱스, 길이) from dual;
...
select substr('good morning john', 8, 4) from dual;--rnin(8번째 문자부터 4칸입력)
select substr('good morning john', 8) from dual;--rning john 길이가없으면 끝까지 출력됨
--인덱스2부터 끝까지를 잘라내지만 한글이므로 영문과는 결과가 조금 다르다
select substr('안녕하세요 존',2) from dual;

/*
substrb() : 바이트(byte)단위로 잘라낸다.
    통상적으로 한글과 같은 유니코드는 한글자에 3byte로 표현되나
    조금씩 어긋나느 경우가 생기므로 테스트가 필요하다.
*/
...
replace() : 문자열을 다른 문자열로 대체할때 사용한다.
    ...
    형식] replace(칼럼명 or 문자열, '변경할 대상의 문자', '변경할 문자')
    
    ※trim(),ltrim(), rtrim()메소드의 기능을 replace()메소드 하나로 대체할수 있으므로
...
형식1] instr(컬럼명, '찾을문자')
    =>문자열의 처음부터 문자를 찾는다.
형식2] instr(컬럼, '찾을문자', 탐색시작위치, 몇번째문자) 
    => 탐색할 인덱스부터 문자를 찾는다. 단, 찾는 문자중 몇번째에 있는 문자인지
    지정할 수 있다.
*/
--n이 발견된 첫번째 인덱스 반환
select instr('good morning tom' , 'n') from dual;--오라클은 1부터 시작이얌, 0부터 시작아니야
--인덱스1부터 탐색을 시작해서 n이 발견된 두번째 인덱스 반환
select instr('good morning tom' , 'n', 1, 2) from dual;
--인덱스 8부터 탐색을 시작해서 m이 발견된 첫번째 인덱스

...
select employee_id, first_name, email, department_id from employees;
...
AS(알리아스) : 테이블 혹은 컬럼에 별칭(별명)을 부여할때 사용한다.
    ...
    활용법] 차후2개이상의 테이블을 JOIN(조인)해야할경우 컬럼명이
        중복될때 구분하는 용도로 사용한다.
*/
...
from employess where first_name='william';

--오라클은 기본적으로 대소문자를 구분하지 않는다. 예약어는 둘다 사용할수 있다.
SELECT employee_id "사원번호", first_name, 
email from employees WHERE first_name='Alexander';

--단, 레코드인 경우 대소문자를 구분한다. 아래 SQL문에서는 아무런 결과가 출력되지 않는다
select employee_id "사원번호", first_name, email from employees 
    where first_name='ALEXander';--이름의 대소문자가 잘못입력됨
    
/*
where절을 이용해서 조건에 맞는 레코드 추출하기
    : last_name이 Smith인 레코드를 추출한다.
    주의] where절에 조건을 입력할때 컬럼이 문자형이면
        싱글코테이션을 사용해야 한다. 숫자형인 경우 생략할수있다.
*/
...
--입사일이 04년01년01일 이후인 사원정보를 추출하시오.
...
--방법1 : or을 사용한다. 컬럼명이 반복적으로 기술해야한다
...
/*
distinct : 칼럼에서 중복되는 레코드를 제거할때 사용한다.
...
/*
사원정보 테이블에서 급여가 낮은 순서에서 높은순서로 나오도록 정력하여 조회하시오
...
/*
부서번호를 내림차순으로 정렬한후 해당 부서에서 낮은 급여를 받는 직원이
...
is null 혹은 is not null
    ...
    컬럼중에 null값을 허용하는 경우 값을 입력하지 않으면 
    null값이 되는데 이를 대상으로 select할때 사용한다.
*/
--보너스율이 없는 사원을 조회하시오
...
#03학생전용 > 02Oracle > 02SelectBasic 폴더하위에 본인이름으로 업로드 합니다.
...
--정렬시 물리적으로 존재하지 않는 칼럼이라면 계산식 그대로를 order by 절에 기술한다.
...
--안되네,ㅋㅋselect ename, job, hiredate from emp where hiredate between (81/02/20 and 81/05/01);

/************************
파일명: Or04TypeConver.sql
형변환함수/ 기타함수
설명 : 데이터타입을 다른 타입으로 변환해야 할때 사용하는 함수와 기타함수
**************************/
/*
sysdate : 현재 날짜와 시간을 초단위로 반환해준다. 주로 게시판에서
...
/*
select
    to_char(sysdate, '오늘은 yyyy년 mm월 dd일 입니다') "과연될까?"
from dual;--에러발생 : 날짜형식이 부적합함.
*/

--서식문자가 아닌 나머지쩌리들을 더블 쿼텐션으로 묶어야한다.
select
...
select
    ...
    to_char(2000000, '999,999,000'),
    ltrim(to_char(2000000, '999,999,000'))"좌측공백제거",
...
select
    ...
    to_date('2021-10-14') "날짜기본서식2",
    to_date('2021-10-14') "날짜기본서식3"
from dual;

/*
날짜포맷이 년-월-일 순이 아닌 경우는 오라클이 인식하지 못하여 에러가
발생한다. 이때는 날짜서식을 이용해서 오라클이 인식할 수 있도록 처리해야한다
*/
...
/*
퀴즈 ] 문자열 '2021/05/05'는 어떤 요일인지 변환함수를 통해 출력하시오.
*/
select
   to_char(sysdate,'day') "오늘의 요일",
    to_date('2021/05/05')"1단계 : 날짜서식인식",
    to_char(to_date('2021/05/05'), 'day') "2단계:요일서식"
from dual;
...
시나리오] '2015-10-24 12:34:56' 형태로 주어진 데이터를 인자로 하여
    '0000년00월00일 0요일'형식으로 변환함수이용 출력.
*/
select
...
nvl() : null값을 다른 데이터로 변경하는 함수
    ...
    ※레코드를 select해서 웹브라우저에 출력을 하는 경우 해당 칼럼이 null이면
    nullpointerException이 발생하게 된다. 그러므로 아예 데이터를 가져올때
    null값이 나올수 있는 칼럼에 대해 미리 처리하면 예외발생을 미리 차단할 수
    있으므로 편리하다
*/
...
/*
decode() : java의 switch문과 비슷하게 특정값에 해당하는 출력문이 있는경우 사용한다.
    형식] decode(칼럼명
                값1, 결과1, 값2, 결과2, ......
                기본값)
...
#03학생전용 > 02Oracle > 03TypeConvert 폴더하위에 본인이름으로 업로드 합니다.
...
/*
5. 올해 며칠이 지났는지 출력하시오. 
현재 날짜에서 올해 1월1일을 뺀 결과를 출력하고 TO_DATE()함수를 사용하여 데이터 형을 일치 시키시오.
단, 날짜의 형태는 ‘01-01-2020’ 포맷으로 사용한다. 즉 sysdate - ‘01-01-2020’ 이와같은 연산이 가능해야한다. */
select 
   to_date('01-01-2020','dd-mm-yyyy') "날짜서식변경",
   sysdate - to_date('01-01-2020', 'dd-mm-yyyy') "날짜의 차",
   round(sysdate - to_date('01-01-2020', 'dd-mm-yyyy')) "결과1",
   trunc(sysdate - to_date('01-01-2020', 'dd-mm-yyyy')) "결과2"
from dual;
/*

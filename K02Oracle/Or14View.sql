...
as 
    ...
    from employees where job_id='ST_CLERK';
/*
    뷰 생성시 칼럼명을 지정하면 마치 별칭을 준것과 같이 기존
    테이블의 컬럼명을 변경할 수 있다.
*/
select * from view_employees;
...
퀴즈] 위에서 생성한 view_employees 뷰를 아래 조건에 맞게 수정하시오
        직무아이디가 ST_MAN인 사원의 사원번호, 이름, 이메일, 매니저아이디를
        조회할 수 있도록 수정하시오.
        뷰의 컬럼명은 e_id, name, m_id로 지정한다. 단, 이름은 first_name과
...
/* 내답
create or replace view view_employees (e_id, name, m_id)
as 
...
as 
  select employee_id, concat(first_name||' ', last_name), (sal+(sal*commission_pct))*12
    from employees ;

 select employee_id, concat(first_name||' ', last_name), annual_sal
    from employees ;
...
--step1 선생님버전
select 
    employee_id, first_name, trim(to_char((salary+(salary*nvl(commission_pct, 0)))*12, '999,000'))
from employees;/*
보너스율이 null값인 경우 연봉이 계산되지 않으므로 nvl()함수를
이용해서 0으로 값을 변환한후 계산한다. 또한 to_char()로

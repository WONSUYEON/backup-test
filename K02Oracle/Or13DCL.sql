...
/*
[사용자계정 생성 및 권한설정]
해당 내용은 DBA권한이 있는 최고관리자(SYS, SYSTEM)으로 접속한 후
실행해야 한다.
...
--생성직후 cmd에서 sqlplus 명령으로 접속시 login denied 에러발생됨
...
--테이블 생성을 해보면 users 테이블 스페이스에 대한 권한이 없어서 생성할 수 없음.

--test_user2가 사용하는 테이블스페이스 확인
select username, default_tablespace from dba_users
    where username in upper('test_user2');--우리가 설정한 대로 user를 사용하고 있음

--users테이블 스페이스에 10m 공간을 할당
...
select username, default_tablespace from dba_users 
 where username in upper('test_user1');--users로 변경된것을 확인
    
/*
sqlplus 에서 ED명령어 사용하기

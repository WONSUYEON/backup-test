...
파일명 : Or05Date.sql
날짜함수
 설명 : 년, 월, 일, 시,,분,초 의 포맷으로 날짜 형식을 지정하거나
    날짜를 계산할때 활용하는 함수들
************************/
...
from dual;--라운드나 세일이나 플로어나 알아서 쓰시오
...
*/
--현재를 기중으로 7개월 이후의 날짜를 구하시오.
select sysdate, add_months(sysdate, 7) as "7개월후"
from dual;
...
select
last_day('20/02/01')
from dual;--20년기준은 윤년이므로 29일 출력됨

select
last_day('21/10/15')
from dual;--중간날짜로 쳐도 그 해당월의 날짜 나옴

--칼럼이 date형인 경우 간단한 날짜연산이 가능하다.
select
sysdate"현재날짜",
sysdate+1 "내일날짜",
sysdate-1 "어제날짜",

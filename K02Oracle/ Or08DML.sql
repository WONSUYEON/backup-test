...
레코드 입력하기 : insert
    ...
    숫자형은 '' 없이 그냥 쓰면된다. 만약 숫자형을 ''로 감싸게 된면
    자동으로 형변환되어 입력된다.
*/
--실습을 위한 테이블 생성
...
커밋을 수행하지않으면 외부에서는 변경된 레코드를 확인할 수 없다.
여기서 말하는 외부란 JAVA/JSP와 같은 오라클 이외의 프로그램을 말한다.
...
--오라클에서 확인하면 실제 삽입된것 처럼 보인다. 하지만 실제 반영되지 않은 상태이다
select * from tb_sample;
...
레코드 삭제하기 : delete
    형식]
        ...
        ※ 레코드를 삭제하므로 delete 뒤에 칼럼을 명시하지 않는다.
*/
--부서번호가 10인 레코드를 삭제하시오
delete from tb_sample where dept_no=10;
--update와 마찬가지로 where절이 없으면 모든레코드가 삭제된다.(주의요망)
...
DML문 : 레코드를 입력 및 조작하는 쿼리문
    ...
    레코드 수정 : update 테이블명 set 칼럼=값 where 조건

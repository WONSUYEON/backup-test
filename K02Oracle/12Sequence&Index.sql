...
2. NoCycle로 설정하고 시퀀스를 계속 얻어올때 MaxValue에 지정값을 초과하면 에러가 발생된다.
...
insert into tb_goods values (1, '새우깡');--프라이머리키라서 같은값 입력안돼!
...
/*
    시퀀스 생성후 최초실행시에는 오류가 발생된다. nextval을 먼저
    실행한 후 실행해야 문제없이 출력된다.
    마지막에 출력된 시퀀스가 출력된다.
*/
select seq_serial_num.currval from dual;
/*
    다음 입력할 일련번호(시퀀스)를 반환한다. 실행할때 마다 다음으로
    넘어간다.
*/
...
insert into tb_goods values (seq_serial_num.nextval, '초콜렛13');/*
        ...
        일련번호가 생성되므로 무결성 제약조건에 위해된다. 즉 기본키에
        사용할 시퀀스는 cycle옵션을 사용하지 않아야한다.
*/
...
--인덱스 수정 : 인덱스는 수정이 불가능하다. 따라서 삭제후 다시 생성해야한다.

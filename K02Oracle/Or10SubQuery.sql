...
/*
시나리오] 사원테이블에서 전체사원의 평균급여보다 낮은급여를 받는
사원들을 추출하여 출력하시오
...
/*
시나리오] 평균급여보다 많은 급여를 받는 사원들의 명단을 조회할수있는
서브쿼리문을 작성하시오
...
--3단계 : 둘중 하나만 만족하면 인출할것이므로 any를 이용해 서브쿼리를 작성
...
Top쿼리 : 조회된 결과에서 구간을 정해 레코드를 가져올때 사용한다.
    ...
    가상의 컬럼을 말한다. 해당 칼럼은 테이블에 존재한다.
*/    
--모든 레코드를 가져온다.
select * from employees;
--정렬없이 모든레코드를 가져와서 rownum을 출력한다(rownum이 순서대로 나옴)
...
/*
    ...
    3.2의 결과 전체를 가져온 다음...
        (2.1의 결과에 rownum을 재 부여한다.
            (1. 사원테이블의 모든 레코드를 오름차순으로 정렬해서 인출)
         )
...
/*
    평균급여는 물리적으로 존재하는 칼럼이 아니므로
    where절에는 사용할 수 없고 having절에 사용해야한다
...
/*
    or 조건을 in으로 표현할 수 있으므로, 서브쿼리에서 복수행 연산자인
...
/*
09.평균 급여 보다 많은 급여를 받고 이름에 K가 포함된 사원과 같은 부서에서 근무하는 사원의 사원번호, 이름, 급여를 표시하시오.
*/
--평균급여

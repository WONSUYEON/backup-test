...
alter table tb_primary3 add constraint tb_primary3_pk
    primary key(user_name);--제약명을 부여하면 오류발생시 해당 이름을 통해 확인할 수 있다.
desc tb_primary3;
select * from user_constraints;
...
insert into tb_unique 
    values (4, '정우성', '010-4444-4444', '영화배우');
insert into tb_unique 
    values (5, '이정재', '010-5555-5555', '영화배우');--입력성공
insert into tb_unique 
    values (6, '황정민', '010-4444-4444', '영화배우');--입력실패
    
/*
    telephone과 nickname 컬럼이 동일한 제약명으로 설정되었으므로
    두개의 컬럼이 동시에 동일한 값을 가지는 경우가 아니라면
    중복된 값이 허용된다.
...
-만약 테이블간에 외래키가 설정되어 있다면 자식테이블에 
참조값이 존재할 경우 부모테이블의 레코드는 삭제 할 수 없다.
...
--부모키가 없으므로 입력불가
insert into tb_foreign1 values(2, '트와이스조아요', '  Twice');
...
*/
delete from tb_primary2 where idx=1;--아직 자식레코드가 살아있으므로 에러발생
...
외래키 삭제시 옵션
[on delete cascade]
    : 부모레코드 삭제시 자식레코드까지 삭제됨
    형식]
        컬럼명 자료형 references 부모테이블(pk칼럼)
            on delete cascade;
[on delete set null]
...
insert into tb_foreign4 values (1, '스팸1입니다', 'kosmo');--그후에 자식테이블을 입력한다.
...
insert into tb_foreign5 values (1, '스팸1입니다', 'kosmo');--그후에 자식테이블을 입력한다.
...
create table tb_not_null (
    m_idx number(10) primary key, --pk(프라이머리 키!)이므로 NN(낫 널)
...
/*
    위에서 보듯이 default 값을 입력하려면 insert시 칼럼자체를
    제외하거나, default 키워드를 사용해야 한다
*/

/*
check : Domain(자료형) 무결성을 유지하기 위한 제약조건으로
    해당 칼럼에 잘못된 데이터가 입력되지 않도록 유지하는 제약조건이다.
*/
...
select * from user_constraints where constraint_name=upper('pr_emp_pk');
...
*/
alter table pr_emp_const /* 외래키는 항상 자식테이블에서 생성한다 */
    add constraint pr_emp_dept_fk /* 제약조건명 추가 */
    foreign key(deptno) /*자식테이블의 외래키(참조키)컬럼 지정*/
    references pr_dept_const (deptno); /*부모테이블의 기본키(PK)컬럼지정 */
    
/*
4. pr_emp_const 테이블의 comm 칼럼에 0보다 큰 값만을 입력할수 있도록 제약조건을 지정하시오.
제약조건명은 지정하지 않아도 된다.
*/
...
insert into pr_emp_const values     
    (200, '머시기', '냉정', null, sysdate, 2000, 0, 10);--입력실패(check제약조건 위배)
...
/*
5. 위 3번에서는 두 테이블간에 외래키가 설정되어서 pr_dept_const 테이블에서 레코드를 삭제할 수 없었다.
...
delete from pr_dept_const; --자식레코드가 있으므로 삭제불가. 외래키 제약조건 위배.

--외래키 제약조건을 재설정하기위해 기존의 제약조건 삭제
...
foreign key (deptno)
references pr_dept_const (deptno) on delete cascade ;

delete from pr_dept_const;-- 자식레코드까지 모두 삭제됨

select * from pr_dept_const;
select * from pr_emp_const;--테이블확인

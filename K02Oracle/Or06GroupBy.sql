...
파일명: Or06GroupBy.sql
그룹함수(select문 2번째)
설명 : 전체레코드(로우)에서 통계적인 결과를 구하기 위해 하나이상의
...
/*
sum(): 합계를 구할때 사용하는 함수
    -number 타입의 칼럼에서만 사용할 수 있다.
    -필드명이 필요한 경우 as 를 이용해서 별칭을 부여할 수 있다.
*/
--전체직원의 급여의 합계를 출력하시오
select
sum(salary)"급여합1", 
...
--sum()과 같은 그룹함수는 number타입인 칼럼에서만 사용할 수 있다.
...
/*
    count()함수를 사용할때는 위2가지 방법 모두 가능하지만
...
count()함수의
    사용법1 : count(all 칼럼명)
    => 디폴트 사용법으로 칼럼 전체의 레코드를 기준으로 카운트한다.
    사용법2 : count(distinct 칼럼명)
    => 중복을 제거한 상태에서 카운트 한다.
*/
...
salary = 2100 ; /* 위에서 구한 급여의 최소값을 조건으로 다시한번 select해야한다*/   
...
/*
group by: 여러개의 레코드를 하나의 그룹으로 그룹화하여 묶여진
    결과를 반환하는 쿼리문.
...
select department_id, sum(salary), to_char(sum(salary),'$999,000') from employees
...
*/
select department_id as"부서번호", sum(salary) as"급여총합", 
count(*)as "사원총합", to_char(avg(salary),'999,000.00') as"평균급여"
from employees group by department_id order by department_id asc;

/*
위에서 사용했던 쿼리문을 아래와 같이 수정하면 에러가 발생한다.
group by절에서 사용한 칼럼은 select절에서 사용할수 있으나,
그외의 단일칼럼은 select절에서는 사용할 수 없다.
...
/*
시나리오] 부서별 아이디가 50인 사원들의 직원총합, 평균급여, 급여총합이
...
to_char(sum(salary), '9,999,000') "급여총합"
from employees where department_id=50
group by department_id;

/*
having절 : 물리적으로 존재하는 칼럼이 아닌 그룹함수를 통해 논리적으로
생성된 컬럼의 조건을 추가할때 사용한다.
해당조건을 where절에 추가하면 에러가 발생한다
*/
/*
시나리오] 사원테이블에서 각 부서별로 근움하고 있는 직원의 담당업무별
...
#과제전용 > 02Oracle > 연습문제2(GroupBy) 폴더하위에 본인이름으로 업로드 합니다.
...
select
max(salary)MayPay, min(salary) MinPay, round(avg(salary)) AvgPay
from employees;
/*
...
select
  ...
  ltrim(to_char(max(Salary), '999,000')) maxpay,
   ltrim(to_char(MIN(Salary), '999,000'))  minpay ,
    ltrim(to_char(sum(salary), '999,000')) avgpay,
     ltrim(to_char(avg(salary), '999,000'))  sumpay
from employees
group by job_id;

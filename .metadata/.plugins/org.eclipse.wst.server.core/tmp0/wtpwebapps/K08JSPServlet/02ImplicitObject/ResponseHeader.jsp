...
파일 다운로드 구현시 사용하는 설정
 ...
 jpg와 같은 이미지 파일은 웹브라우저가 인식하는 MIME타입이으로 다운로드를
...
<body>
		<h2>응답 헤더 정보 출력하기</h2>
		<%
		//getHeaderNames() 메소드를 통해 응답헤더를 얻어온다
		...
		동일한 값이 출력된다. 이것은 getHeader() 메소드의 특성인데
		처음 입력한 헤더값만 출력되게 된다.
		*/
		%>
		...
		<%
		/*
		myNumber 라는 헤더명으로 2개의 값을추가했으므로 아래에서는
		각각의 값이 정상적으로 출력된다. 즉, add 계열의 메소드는 헤더명을
